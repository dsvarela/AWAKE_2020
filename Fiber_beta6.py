...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        [self.LEdge, self.DutyT,
         self.FiberCheck, self.InternalF] = ([] for i in range(4))
        ...
        for i in range(self.num_fibers):
            ...
            self.FiberCheck.append(tk.DoubleVar())
            self.FiberCheck[i].set(0)

            self.InternalF.append(tk.DoubleVar())
            self.InternalF[i].set(0)
    ...
    def Frame_Contents(self, parent):
        ...
        def upload_master():
            ...
            [error, loop_n, per_cnt, lead_cnt,
             trail_cnt] = calc(milliseconds.get(), f_lead, f_duty)
